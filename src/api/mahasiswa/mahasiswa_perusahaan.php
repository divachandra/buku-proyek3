...
$m = new Mahasiswa();
if (isset($_POST['id_intern'], $_POST['id_mahasiswa'], $_POST['id_pembimbing'], $_POST['latitude_perusahaan'], $_POST['longitude_perusahaan'], $_FILES['foto_perusahaan']['name'])) {
    ...
    if (!empty($id_intern) && !empty($id_mahasiswa) && !empty($id_pembimbing) && !empty($latitude_perusahaan) && !empty($longitude_perusahaan) && !empty($foto_perusahaan)) {
        if ($m->get_distance($id_pembimbing) == false) {
            ...
            if ($gap>30) {
                $json['status']  = 300;
                $json['message'] = $gap.'Jarak terlalu jauh';
                echo json_encode($json);
            } else {
                $registrasi_perusahaan = $m->registrasi_perusahaan($id_intern, $id_mahasiswa, $id_pembimbing, $latitude_perusahaan, $longitude_perusahaan, $foto_perusahaan);
