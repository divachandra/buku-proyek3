...
class Pembimbing{
    ...
    public function registrasi_pembimbing($nama_pembimbing, $kontak_pembimbing, $nama_perusahaan, $username, $password, $latitude_perusahaan, $longitude_perusahaan){
        ...
        if (mysqli_num_rows($result1) > 0 && mysqli_num_rows($result2) == 0) {
            $query1 = "INSERT INTO pembimbing_perusahaan (nama_pembimbing, nama_perusahaan_perusahaan, lat_pembimbing_perusahaan, long_pembimbing_perusahaan, kontak_pembimbing) VALUES ('$nama_pembimbing', '$nama_perusahaan', '$latitude_perusahaan', '$longitude_perusahaan', '$kontak_pembimbing')";
            $query2 = "INSERT INTO user VALUES ((SELECT LAST_INSERT_ID(id_pembimbing) FROM pembimbing_perusahaan ORDER BY id_pembimbing DESC LIMIT 1), '$username', '$password', 'tidak aktif', 'pembimbing')";
            $result1 = mysqli_query($this->connection, $query1);
            $result2 = mysqli_query($this->connection, $query2);
            if ($result1 && $result2) {
                $json['status']  = 200;
            ...
            $query1 = "INSERT INTO pembimbing_perusahaan (id_pembimbing, nama_pembimbing, nama_perusahaan_perusahaan, lat_pembimbing_perusahaan, long_pembimbing_perusahaan, kontak_pembimbing) VALUES (190001, '$nama_pembimbing', '$nama_perusahaan', '$latitude_perusahaan', '$longitude_perusahaan', '$kontak_pembimbing')";
            $query2 = "INSERT INTO user VALUES ((SELECT LAST_INSERT_ID(id_pembimbing) FROM pembimbing_perusahaan ORDER BY id_pembimbing DESC LIMIT 1), '$username', '$password', 'tidak aktif', 'pembimbing')";
    ...
    public function login_pembimbing($user, $pass){
        $query = "SELECT id_pembimbing, username, password, nama_pembimbing, nama_perusahaan_perusahaan, kontak_pembimbing, foto_pembimbing FROM user INNER JOIN pembimbing_perusahaan ON user.id_user = pembimbing_perusahaan.id_pembimbing WHERE username='$user' AND password='$pass' AND status_user='aktif' AND role_user='pembimbing'";
    ...
    public function list_konfirm_mahasiswa($id_pembimbing) {
        $query = "SELECT id_intern, id_mahasiswa, id_dosen_dt_intern as id_dosen, nama_mahasiswa, foto_mahasiswa, kontak_mahasiswa, nama_prodi FROM detail_intern INNER JOIN intern ON detail_intern.id_intern_dt_intern = intern.id_intern INNER JOIN mahasiswa ON detail_intern.id_mahasiswa_dt_intern = mahasiswa.id_mahasiswa INNER JOIN prodi ON mahasiswa.id_prodi_mahasiswa = prodi.id_prodi WHERE id_pembimbing_dt_intern='$id_pembimbing' AND intern.status_intern='berjalan' AND (status_perusahaan IS NULL OR status_perusahaan = '')";
        $result = mysqli_query($this->connection, $query);
        if (mysqli_num_rows($result) > 0) {
            $datas = array();
            while ($row  = mysqli_fetch_array($result)) {
    ...
    public function list_notifikasi($id_pembimbing){
        $query = "SELECT id_intern, nama_mahasiswa, pesan_notifikasi, tgl_waktu_notifikasi FROM notifikasi INNER JOIN pembimbing_perusahaan ON id_pembimbing = id_pembimbing_notifikasi INNER JOIN mahasiswa ON id_mahasiswa = id_mahasiswa_notifikasi INNER JOIN intern ON id_intern = id_intern_notifikasi WHERE id_pembimbing_notifikasi='$id_pembimbing' AND status_intern='berjalan' ORDER BY tgl_waktu_notifikasi DESC";
        $result = mysqli_query($this->connection, $query);
        if (mysqli_num_rows($result) > 0) {
            $datas = array();
    ...
    public function konfirm_mahasiswa($id_intern, $id_pembimbing, $id_mahasiswa, $id_dosen){
        $query1 = "UPDATE detail_intern SET status_perusahaan='diterima' WHERE id_pembimbing_dt_intern='$id_pembimbing' AND id_mahasiswa_dt_intern='$id_mahasiswa' AND id_intern_dt_intern='$id_intern' AND (status_perusahaan IS NULL OR status_perusahaan = '')";
        $result1 = mysqli_query($this->connection, $query1);
        $tgl_waktu = date("Y-m-d h:i:s");
        $query2 = "INSERT INTO notifikasi(id_intern_notifikasi, id_mahasiswa_notifikasi, id_dosen_notifikasi, id_pembimbing_notifikasi, pesan_notifikasi, tgl_waktu_notifikasi) VALUES ('$id_intern', '$id_mahasiswa', '$id_dosen', '$id_pembimbing', 'mengkonfirmasi pendaftaran', '$tgl_waktu')";
        $result2 = mysqli_query($this->connection, $query2);
    ...
    public function konfirm_absensi($id_intern, $id_pembimbing, $id_mahasiswa, $id_dosen, $id_absensi){
        $query1 = "UPDATE absensi SET status_absensi='diterima' WHERE id_pembimbing_absensi='$id_pembimbing' AND id_mahasiswa_absensi='$id_mahasiswa' AND id_intern_absensi='$id_intern' AND id_absensi='$id_absensi'";
        $result1 = mysqli_query($this->connection, $query1);
        $tgl_waktu = date("Y-m-d h:i:s");
        $query2 = "INSERT INTO notifikasi(id_intern_notifikasi, id_mahasiswa_notifikasi, id_dosen_notifikasi, id_pembimbing_notifikasi, pesan_notifikasi, tgl_waktu_notifikasi) VALUES ('$id_intern', '$id_mahasiswa', '$id_dosen', '$id_pembimbing', 'mengkonfirmasi absensi', '$tgl_waktu')";
        $result2 = mysqli_query($this->connection, $query2);
        if ($this->connection->affected_rows > 0 && $result1 && $result2) {
            $json['status']  = 200;
